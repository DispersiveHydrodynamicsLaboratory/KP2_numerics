...
check_IC = 1;  % Set to nonzero to plot the ICs and BCs without running the solver
    ...
    Nx     = 2^10;    % Number of Fourier modes in x-direction
    ...
    if save_on
        if plot_on
            ...
            subplot(2,2,2)
                ...
                title('Asymptotic u');
            subplot(2,2,3)
                xerr = abs(soli.uax(XPLOT(:,1:end-1),YPLOT(:,1:end-1),5)-diff(soli.ua(XPLOT,YPLOT,5),1,2)./diff(XPLOT,1,2));
                contourf(XPLOT,YPLOT,soli.uax(XPLOT,YPLOT,0),100,'edgecolor','none'); xlabel('x'); ylabel('y'); 
                title(['Asymptotic u, x-deriv err ',num2str(max(xerr(:)),'%10.2e')]);
            subplot(2,2,4)
                yerr = abs(soli.uay(XPLOT(1:end-1,:),YPLOT(1:end-1,:),5)-diff(soli.ua(XPLOT,YPLOT,5),1,1)./diff(YPLOT,1,1));
                contourf(XPLOT,YPLOT,soli.uay(XPLOT,YPLOT,0),100,'edgecolor','none'); xlabel('x'); ylabel('y'); 
                title(['Asymptotic u, y-deriv err ',num2str(max(yerr(:)),'%10.2e')]);
