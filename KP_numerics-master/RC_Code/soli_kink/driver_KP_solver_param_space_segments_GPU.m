...
%         ic_type = ['KP2_validation_Nexp_',num2str(Nexp),'_dt_',num2str(Nt),'_twosoli'];
        ...
        sau = 1; sad = sads(ii);
        qau = 0; qad = qads(ii);
            x0 = 100; y0 = 0; x0odd = -100; w = Ly*2/3;
            tanw = 1/20;
%% Define Soliton parameters and derivatives in x and y
    ...
    soli.a     = @(x,y,t) gpuArray(( (sad^2-sau^2)./2.*tanh(soli.tw.*(y)) + (sad^2+sau^2)/2 )) ;
    soli.ax    = @(x,y,t) gpuArray(zeros(size(x)));
    soli.ay    = @(x,y,t) gpuArray((sad^2-sau^2)/2*soli.tw*sech(soli.tw*(y)).^2);
