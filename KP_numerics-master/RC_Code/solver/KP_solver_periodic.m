...
domain = struct;
    domain.x = (2*Lx/Nx)*(-Nx/2:Nx/2-1)';
   domain.dx = (2*Lx/Nx);
   domain.Lx = Lx;
    domain.y = (2*Ly/Ny)*(-Ny/2:Ny/2-1)';
   domain.dy = (2*Ly/Ny);
   domain.Ly = Ly;
    [domain.X,domain.Y] = meshgrid(domain.x,domain.y);
    domain.k = (pi/Lx)*[0:Nx/2-1 0 -Nx/2+1:-1]';
    domain.l = (pi/Ly)*[0:Ny/2-1 0 -Ny/2+1:-1]';
...
%% Write static matrices here, so not constantly redefining  
    ...
    W.o = exp( -an * abs(domain.Y/Ly).^n );
    W.p = -an*n/Ly * W.o .* abs(domain.Y/Ly).^(n-1).*sign(domain.Y/Ly);
    W.pp = an*n/Ly^2 * W.o .* abs(domain.Y/Ly).^(n-2) .* ...
