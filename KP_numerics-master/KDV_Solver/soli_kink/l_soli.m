function [ soli ] = l_soli( sau, sad, qau, qad, x0, y0, Lx )
...
%% Define Soliton parameters and derivatives in x and y
    soli.a     = @(x,y,t) (sad-sau)/2*tanh(1/5*(y-0)) + (sad+sau)/2;%qa(x,y,t);% qa   .* ones(size(x));
    soli.ax    = @(x,y,t) zeros(size(x));
    soli.ay    = @(x,y,t) (sad-sau)/2*1/5*sech(1/5*(y-0)).^2;
    soli.q     = @(x,y,t) (qad-qau)/2*tanh(1/5*(y-0)) + (qad+qau)/2;%qa(x,y,t);% qa   .* ones(size(x));
    soli.qx    = @(x,y,t) zeros(size(x));
    soli.qy    = @(x,y,t) (qad-qau)/2*1/5*sech(1/5*(y-0)).^2;
    soli.x0    = x0;
    soli.y0    = y0;
